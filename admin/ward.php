...
<head>
<title><?php echo $Proj_Title; ?> | Ward No</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Ward No
...
<span class="font-weight-light">Ward No</span>
</h5>
...
<?php 
        $q = "select * from tbl_city WHERE Status='1' AND StateId=1";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
<?php 
        $q = "select * from tbl_zone WHERE Status='1' AND StateId=1 AND CityId=819";
        $r = $conn->query($q);
        while($rw = $r->fetch_assoc())
    {
?>
...
<input type="text" name="Name" class="form-control" id="Name" placeholder="Ward No Name" required>
<div class="clearfix"></div>
</div>
</div>
...
function category_lists(){
  ...
  success: function(data){
    ...
    function getZone(CityId,ZoneId){
      var action = "getZone";
        $.ajax({
    url:"ajax_files/ajax_dropdown.php",
    method:"POST",
    ...
    {
      $('#ZoneId').html(data);
        $('#ZoneId').val(ZoneId).attr("selected",true);  
    }
    });
    }
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Ward No Already Exists',
   ...
   $.growl.success({
      title:    'Success',
      message:  'New Ward No Added Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
function update_toast(){
   ...
   $.growl.success({
      title:    'Success',
      message:  'Ward No Updated Successfully!',
      location: isRtl ? 'tl' : 'tr'
    });
  }
   ...
   var action = "getZone";
    ...
    success:function(data)
    {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Ward No</span>");  
           $('#action').val("Add");  
           $('#id').val('');
      ...
      $('#Status').attr("selected","selected").val(1);
       $('#CountryId').attr("selected","selected").val(null);
        $('#StateId').val(1).attr("selected",true);
        $('#CityId').val(819).attr("selected",true);
        $('#ZoneId').val(null).attr("selected",true);
       $('#submit').text('Submit');
                ...
                success:function(data){  
                 ...
                 getZone(CityId,ZoneId); 
                     ...
                     $('#action').val('Edit'); 
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Ward No</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "You will not be able to recover this Ward No!",
            type: "warning",
            ...
            if (isConfirm) {
                ...
                success:function(data){
              swal("Deleted!", "Ward No has been deleted.", "success");
              ...
              category_lists();
                ...
                swal("Cancelled", "Ward No is safe :)", "error");
            }
        });
