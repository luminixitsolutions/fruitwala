...
$Status = $_POST["Status"];
$MenuId = $_POST["MenuId"];
$query = "SELECT * FROM tbl_sub_menu WHERE Name = '$Name' AND MenuId='$MenuId'";
...
$qx = "INSERT INTO tbl_sub_menu SET Name = '$Name',MenuId='$MenuId',Status='$Status'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_sub_menu WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_sub_menu WHERE Name = '$Name' AND MenuId='$MenuId' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_sub_menu SET Name = '$Name',MenuId='$MenuId',Status='$Status' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_sub_menu WHERE id = '$id'";
      $conn->query($query);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT sb.*,c.Name As Country FROM tbl_sub_menu sb LEFT JOIN tbl_main_menu c ON c.id=sb.MenuId ORDER BY sb.id DESC";
   $rx = $conn->query($sql);
