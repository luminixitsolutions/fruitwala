...
if($_POST['action'] == 'deletePhoto') {
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
    $query = "UPDATE tbl_study_materials SET Photo='' WHERE id=$id";
    $conn->query($query);
    $src = "../../uploads/$Photo";
    unlink($src);
...
if($_POST['action'] == 'deleteDoc'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_study_materials SET File='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'showMaterial'){
    ...
    $subtopicid = $_POST['subtopicid'];
      $sql22 = "SELECT * FROM tbl_upload_materials WHERE Status=1 AND MatFor=1";
     if($deptid!=''){
      ...
      foreach($row22 as $result22){
        ...
        $sql7 = "SELECT File FROM tbl_study_materials WHERE DeptId='$deptid' AND SubjectId='$subjectid' AND BatchId='$batchid'";
        if($subtopicid!=''){
      $sql7.= " AND SubTopicId='$subtopicid'";  
      }
        $row7 = getRecord($sql7);
