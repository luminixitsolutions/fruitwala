...
$qx = "INSERT INTO tbl_popup_image SET Name = '$Name',Details = '$Details',Status='$Status',Photo='$Photo',Photo2='$Photo2'";
	$conn->query($qx);
	echo 1;
}
...
$ModifiedDate = date('Y-m-d');
  $query2 = "UPDATE tbl_popup_image SET Photo='$Photo' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
       $query2 = "SELECT Photo FROM tbl_popup_image WHERE id = '$id'";
...
if($_POST['action'] == 'deletePhoto2'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_popup_image SET Photo2='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_popup_image WHERE id=1 ORDER BY id DESC";
   $rx = $conn->query($sql);
  ...
  ?>
           <tr>
             ...
             <td><a data-id="<?php echo $nx['id']; ?>" href='javascript:void(0);' data-toggle="tooltip" data-placement="top" title="Edit" data-original-title="Edit" class="update"><i class="lnr lnr-pencil mr-2"></i></a>
             </td>
            </tr>
             <?php $srno++;} ?>
        </tbody>
