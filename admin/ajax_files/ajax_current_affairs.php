...
if($_POST['action'] == 'deletePhoto') {
   	$id = $_POST['id'];
   	$Photo = $_POST['Photo'];
    $query = "UPDATE tbl_current_affairs SET Photo='' WHERE id=$id";
    $conn->query($query);
    $src = "../../uploads/$Photo";
    unlink($src);
...
if($_POST['action'] == 'deleteDoc'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_current_affairs SET File='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'showMaterial'){
    ...
    $subtopicid = $_POST['subtopicid'];
      $sql22 = "SELECT * FROM tbl_upload_materials WHERE Status=1 AND MatFor=5";
      if($deptid!=''){
      ...
      foreach($row22 as $result22){
        $sql7 = "SELECT File FROM tbl_current_affairs WHERE DeptId='$deptid' AND SubjectId='$subjectid' AND SubTopicId='$subtopicid' AND BatchId='$batchid'";
        $row7 = getRecord($sql7);
...
if($_POST['action'] == 'getAssignDetails'){
  $deptid = $_POST['deptid'];
    ...
    $subjectid = $_POST['subjectid'];
    $subtopicid = $_POST['subtopicid'];
    $query = "SELECT * FROM tbl_current_affairs WHERE DeptId='$deptid' AND SubjectId='$subjectid' AND SubTopicId='$subtopicid' AND BatchId='$batchid'";
...
if($_POST['action'] == 'showSubjects'){
    ...
    $courseid = $_POST['courseid'];
      ...
      foreach($row22 as $result22){
         ...
         $sql7 = "SELECT Subjects FROM tbl_current_affair_subjects WHERE DeptId='$deptid'";
      if($courseid!=''){
...
<?php 
    $DeptId = $_POST['deptid'];
    $CourseId = $_POST['id'];
        $sql= "select Subjects from tbl_current_affair_subjects WHERE DeptId = '$DeptId' AND CourseId='$CourseId'";
        $row = getRecord($sql);
