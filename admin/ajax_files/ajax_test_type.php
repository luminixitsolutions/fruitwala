...
$query = "SELECT * FROM tbl_ts_type WHERE Name = '$Name' AND CourseId='$CourseId'";
...
$qx = "INSERT INTO tbl_ts_type SET Name = '$Name',CourseId='$CourseId',DeptId='$DeptId',Status='$Status',Photo='$Photo'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_ts_type WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_ts_type WHERE Name = '$Name' AND CourseId='$CourseId' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
     ...
     mysqli_set_charset('utf8');
  $query2 = "UPDATE tbl_ts_type SET Name = '$Name',CourseId='$CourseId',DeptId='$DeptId',Status='$Status',Photo='$Photo' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_ts_type WHERE id = '$id'";
      $conn->query($query);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_ts_type SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT tc.*,td.Name As Department FROM tbl_ts_type tc LEFT JOIN tbl_courses td ON tc.CourseId=td.id ORDER BY tc.id DESC";
   $rx = $conn->query($sql);
