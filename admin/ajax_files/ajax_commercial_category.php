...
$query = "SELECT * FROM tbl_commercial_category WHERE Name = '$Name'";
...
$qx = "INSERT INTO tbl_commercial_category SET Name = '$Name',Status='$Status',CreatedDate='$CreatedDate',CreatedBy='$user_id'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_commercial_category WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_commercial_category WHERE Name = '$Name' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
  ...
  $query2 = "UPDATE tbl_commercial_category SET Name = '$Name',Status='$Status',ModifiedDate='$ModifiedDate',ModifiedBy='$user_id' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_commercial_category WHERE id = '$id'";
      $conn->query($query);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_commercial_category ORDER BY id DESC";
   $rx = $conn->query($sql);
