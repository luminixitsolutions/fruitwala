...
$qx = "INSERT INTO tbl_home_banners SET Name = '$Name',Status='$Status',Photo='$Photo'";
	$conn->query($qx);
	echo 1;
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_home_banners WHERE id = '$id'";
...
$ModifiedDate = date('Y-m-d');
  $query2 = "UPDATE tbl_home_banners SET Name = '$Name',Status='$Status',Photo='$Photo' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
       $query2 = "SELECT Photo FROM tbl_home_banners WHERE id = '$id'";
    $result2 = $conn->query($query2);
    $row2 = $result2->fetch_assoc();
    $Photo = $row2['Photo'];
      $query = "DELETE FROM tbl_home_banners WHERE id = '$id'";
      $conn->query($query);
       $src = "../../uploads/$Photo";
        unlink($src);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_home_banners SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT * FROM tbl_home_banners ORDER BY id DESC";
   $rx = $conn->query($sql);
  ...
  ?>
           <tr>
             ...
             <td><a href='add-home-banner.php?id=<?php echo $nx['id']; ?>' data-toggle="tooltip" data-placement="top" title="Edit" data-original-title="Edit" class=""><i class="lnr lnr-pencil mr-2"></i></a> &nbsp;&nbsp;<a data-id="<?php echo $nx['id']; ?>" href='javascript:void(0);' data-toggle="tooltip" data-placement="top" title="Delete" data-original-title="Delete" class="delete" id="bootbox-confirm"><i class="lnr lnr-trash text-danger"></i></a> 
             </td>
            </tr>
             <?php $srno++;} ?>
        </tbody>
