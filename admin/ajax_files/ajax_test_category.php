...
$query = "SELECT * FROM tbl_ts_category WHERE Name = '$Name' AND DeptId='$DeptId'";
...
$qx = "INSERT INTO tbl_ts_category SET Name = '$Name',DeptId='$DeptId',Status='$Status',Photo='$Photo'";
	$conn->query($qx);
	echo 1;
}
}
...
if($_POST['action'] == 'fetch_record'){
 $id = $_POST['id'];
    $query = "SELECT * FROM tbl_ts_category WHERE id = '$id'";
    $result = $conn->query($query);
...
$query = "SELECT * FROM tbl_ts_category WHERE Name = '$Name' AND DeptId='$DeptId' AND id != '$id'";
$result = $conn->query($query);
$row_cnt = mysqli_num_rows($result);
if($row_cnt > 0){
  echo 0;
    ...
    mysqli_set_charset('utf8');
    $query2 = "UPDATE tbl_ts_category SET Name = '$Name',DeptId='$DeptId',Status='$Status',Photo='$Photo' WHERE id = '$id'";
 	$conn->query($query2);
  echo 1;
}
}
  ...
  if($_POST['action'] == 'delete') {
      ...
      $id = $_POST['id'];
      $query = "DELETE FROM tbl_ts_category WHERE id = '$id'";
      $conn->query($query);
...
if($_POST['action'] == 'deletePhoto'){
    $id = $_POST['id'];
    $Photo = $_POST['Photo'];
        $q = "UPDATE tbl_ts_category SET Photo='' WHERE id=$id";
        $conn->query($q);
        $src = "../../uploads/$Photo";
        unlink($src);
...
<table id="example" class="table table-striped table-bordered dt-responsive nowrap" style="width:100%">
        ...
        <tbody>
          <?php 
 $srno = 1;
  $sql = "SELECT tc.*,td.Name As Department FROM tbl_ts_category tc LEFT JOIN tbl_departments td ON tc.DeptId=td.id ORDER BY tc.id DESC";
   $rx = $conn->query($sql);
