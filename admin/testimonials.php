...
<head>
<title><?php echo $Proj_Title; ?> | Testimonials</title>
<meta charset="utf-8">
...
<h4 class="font-weight-bold py-3 mb-0">List Of Testimonials
...
<span class="font-weight-light">Testimonials</span>
</h5>
...
<textarea name="Feedback" class="form-control" id="Feedback" placeholder="Feedback" autocomplete="off" required rows="5"><?php echo $row7["Feedback"]; ?></textarea>
<div class="clearfix"></div>
</div>
</div>
...
function category_lists(){
  var action = 'view';
      $.ajax({
  type: "POST",
  url: "ajax_files/ajax_testimonials.php",
   data:{action:action},  
  ...
  function error_toast(){
   ...
   $.growl.error({
      title:    'Error',
      message:  'Testimonials Name Already Exists',
    ...
    function success_toast(){
      /*swal({
            title: "Success!",
            text: "New Testimonials has been Added.",
            type: "success",
...
function update_toast(){
  ...
  $(document).ready(function() {
    ...
    $('#example').DataTable( {
      ...
      $('#add_button').click(function(){  
           $('.modal-title').html("Add <span class='font-weight-light'>Testimonials</span>");  
           $('#action').val("Add");  
           $('#id').val('');
 ...
 $.ajax({  
                ...
                success:function(data){  
                    ...
                    $('#show_photo').html('<div class="ui-feed-icon-container float-left pt-2 mr-3 mb-3"><a href="javascript:void(0)" class="ui-icon ui-feed-icon ion ion-md-close bg-secondary text-white" id="delete_photo"></a><img src="../uploads/'+data.Photo+'" alt="" class="img-fluid ticket-file-img" style="width: 64px;height: 64px;"></div>');
                       ...
                       $('#submit').text("Update");   
                       $('.insert_frm').modal('show');
                         $('.modal-title').html("Update <span class='font-weight-light'>Testimonials</span>"); 
 ...
 //alert(id);
   swal({
            title: "Are you sure?",
            text: "Deleted All Records Related this Testimonials & You will not be able to recover this Testimonials!",
            type: "warning",
            ...
            if (isConfirm) {
              ...
              category_lists();
                ...
                swal("Cancelled", "Testimonials is safe :)", "error");
            }
        });
...
event.preventDefault();  
if(confirm("Are you sure you want to delete Testimonials Photo?"))  
           {  
